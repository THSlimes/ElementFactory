...
export default class ElementFactory {
    ...
    public base<P extends AssemblyLine.Parameters>(defaultParameters: P, href?: Parameters<BaseAssemblyLine<P>["href"]>[0]) {
    ...
    public button<P extends AssemblyLine.Parameters>(defaultParameters: P, onClick?: (ev: MouseEvent, self: HTMLButtonElement) => void, doDisable?: Parameters<ButtonAssemblyLine<P>["isDisabled"]>[0]) {
        let out = new ButtonAssemblyLine(defaultParameters);
        if (onClick !== undefined) out = out.on("click", onClick);
        if (doDisable !== undefined) out = out.isDisabled(doDisable);
    ...
    public optgroup<V extends string, P extends AssemblyLine.Parameters>(defaultParameters: P, label?: Parameters<OptGroupAssemblyLine<P>["label"]>[0], ...options: AssemblyLine.DynamicValue.Either<HTMLOptionElement | OptionAssemblyLine<any, {}>>[]) {
...
export const EF = ElementFactory.INSTANCE;