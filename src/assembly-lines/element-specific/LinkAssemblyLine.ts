...
/**
 * A BasicAssemblyLine for the `<link>` element.
 *
 * @param R the link's relationship (rel attribute)
 */
class LinkAssemblyLine<R extends LinkAssemblyLine.Relationship, P extends AssemblyLine.Parameters> extends FetchingAssemblyLine<HTMLLinkElement, P> {
    ...
    /**
     * Adds a step that determines the link's destination.
    ...
    /**
     * Adds a step that sets the link's type.
