...
/**
 * A BasicAssemblyLine for the `<meta>` element.
 *
 * @param N meta tag name
 */
class MetaAssemblyLine<N extends MetaAssemblyLine.Name, P extends AssemblyLine.Parameters> extends BasicAssemblyLine<HTMLMetaElement, P> {
    ...
    /**
     * Adds a step that sets the meta element's content.
     * @param content value provider
     */
    public content(content: AssemblyLine.DynamicValue.Either<MetaAssemblyLine.ContentForName<N>, HTMLMetaElement, P>) {
        return this.addStep((e, params) =>
            e.content = [AssemblyLine.DynamicValue.resolve(content, e, params)]
                .flat()
                .join(',')
        );
    }
